...
export function NowNoljak() {
  ...
  const filteredCards = activeFilter === "all" ? cards : cards.filter(c => c.platform === activeFilter)
  ...
  return (
    <section id="now-noljak" className="bg-white py-[120px] px-4 md:px-8">
      <div className="max-w-7xl mx-auto flex flex-col items-center gap-12">
              ...
              >
                <div className="relative">
                  <div className="aspect-video overflow-hidden">
                    <img src={card.thumbnail_url || '/images/now-01.jpg'} alt={card.title} className="w-full h-full object-cover" />
                  </div>
                  <span className={`absolute top-3 left-3 px-2 py-1 rounded text-xs font-medium ${platformColors[card.platform] || 'bg-[#0F1B3D] text-white'}`}>
                    {card.platform}
                  </span>
                </div>
        ...
        <a
          href={locale === "en" ? "/now-noljak" : `/${locale}/now-noljak`}
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="flex flex-col gap-4">
              <div className="aspect-video rounded-lg overflow-hidden">
                <img src={selectedCard?.thumbnail_url || '/images/now-01.jpg'} alt={selectedCard?.title} className="w-full h-full object-cover" />
              </div>
              {selectedCard && (
                <span className={`self-start px-3 py-1 rounded text-sm font-medium ${platformColors[selectedCard.platform] || 'bg-[#0F1B3D] text-white'}`}>
              ...
              {selectedCard && (
                <Button asChild className="bg-[#F6C400] text-[#0F1B3D] font-bold hover:bg-[#E5B600] mt-2">
                  <a href={selectedCard.url || platformLinks[selectedCard.platform]} target="_blank" rel="noopener noreferrer">
