...
export default function AdminDashboard() {
  ...
  const menus = [
    ...
    { title: 'Find Center', desc: '교육원 정보 관리', icon: '📍', path: '/admin/centers', disabled: false },
    ...
    { title: '계정 관리', desc: '관리자 계정 추가/삭제', icon: '👤', path: '/admin/accounts', disabled: false },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      <header className="bg-[#0F1B3D] px-8 py-4 flex items-center justify-between">
        ...
        <button onClick={handleLogout} className="text-white hover:text-[#F6C400] text-sm transition">
          로그아웃
        </button>
      </header>
